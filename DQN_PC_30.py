# Test basic movement 30: Collision Avoidance
# RL Framework: Deep Q-learning
# Image as a state
# Saved model: trained_DQN_PC_30.h5
# Action spaces: 8 Dimensions
#                [Left, Forward, Right, Backward, Forward-Right, Forward-Left, Backward-Left, Backward-Right]
# Note: Use DepthImage (Gray-scale), New reward shaping function, Use gradient method instead of fitting.
# Trained episode: 2175
...
class DQLearning:
    def __init__(self):
        ...
        self.next_state_distance = loaded_erb['arr_7']
        self.next_state_loc = loaded_erb['arr_8']
        ...
        self.q_pred = load_model('trained_DQN_PC_30.h5')
    ...
    def action_space(self, action_no):
        ...
        if action_no == 0:
            ...
            # Forward-Left --> move +x/2, -y/2
            self.drone.moveByVelocityAsync(throttle / 2, -throttle / 2, lift_compensation, duration).join()
        elif action_no == 2:
            # Forward-Right --> move +x/2, +y/2
            self.drone.moveByVelocityAsync(throttle/2, throttle/2, lift_compensation, duration).join()
        elif action_no == 3:
            # Left --> move -y
            ...
            # Backward-Left --> move -x/2, -y/2
            self.drone.moveByVelocityAsync(-throttle/2, -throttle/2, lift_compensation, duration).join()
        elif action_no == 7:
            # Backward-Right --> move -x/2 , +y/2
            self.drone.moveByVelocityAsync(-throttle/2, throttle/2, lift_compensation, duration).join()
        else:
            # No action
    ...
    def environment(self, action):
        ...
        path_reward = 100 - (10 * abs(self.final_coordinates[1] - self.next_drone_coordinates[1]))
        ...
        self.next_distance_target = 100 - (100 * dis / self.max_dis)
        return r
    ...
    def action_value_network_training(self, indices):
        ...
        # current_state_loc_batch = np.zeros([self.batch_buffer_size, self.distance_target_dimension], dtype=float)
        ...
        # next_state_loc_batch = np.zeros([self.batch_buffer_size, self.distance_target_dimension], dtype=float)
        ...
        for k in range(self.batch_buffer_size):
            ...
            if self.death_flag_data[indices[k]]:
                ...
                # next_state_loc_predicted = next_state_loc_batch[k]
                # next_state_loc_predicted = np.expand_dims(next_state_loc_predicted, axis=0)
                # next_state_loc_predicted_tensor = tf.convert_to_tensor(next_state_loc_predicted)
        ...
        # current_state_loc_batch_tensor = tf.convert_to_tensor(current_state_loc_batch, dtype=tf.float32)
        ...
        with tf.GradientTape() as tape:
            ...
            # tape.watch(current_state_loc_batch_tensor)
            tape.watch(y_true_tensor)
            y_pred_tensor = self.q_pred([current_state_img_batch_tensor,
                                         current_state_distance_batch_tensor])
        ...
        droneDQL.q_pred.save("trained_DQN_PC_30.h5")  # Save the model
    ...
    def agent_training(self, episode):
        ...
        for i in range(episode):
            ...
            init_y = randrange(0, 60)  # Random y position in the environment
            ...
            epsilon = epsilon - 0.005  # Epsilon decays 0.002 for every episode.
            if epsilon < 0.1:
            ...
            while not self.terminated:
                ...
                if self.step > (self.buffer_size - 1):
                    ...
                    np.savez('ERB_DQN_PC_30', self.current_state_img, self.current_state_distance,
                             self.current_state_loc, self.action_data, self.death_flag_data, self.reward_data,
                             self.next_state_img, self.next_state_distance, self.next_state_loc)
            ...
            self.append_avg_reward.append(np.sum(self.append_reward)/len(self.append_reward))
            np.save('append_reward_30', self.append_reward)
            np.save('append_avg_reward_30', self.append_avg_reward)
...
if __name__ == "__main__":
    ...
    droneDQL.q_pred.save("trained_DQN_PC_30.h5")
# ******************************************* Main Program End *********************************************************
